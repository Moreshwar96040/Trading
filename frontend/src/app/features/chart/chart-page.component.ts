...
const RANGE_DAYS: Record<RangeKey, number> = { '3M': 92, '6M': 183, '1Y': 365, '2Y': 730 };
  ...
  template: `
    <div class="controls">
      <mat-form-field appearance="outline" class="search">
        ...
        <input matInput placeholder="e.g. RELIANCE" [formControl]="searchControl"
    ...
    @if (selectedTicker()) {
      ...
      <p class="hint">Search for a symbol above to load its chart.</p>
    }
  `,
  styles: `
...
export class ChartPageComponent implements OnInit {
  ...
  readonly rangeKeys: RangeKey[] = ['3M', '6M', '1Y', '2Y'];
  ...
  onSymbolSelected(event: MatAutocompleteSelectedEvent): void {
    this.selectedTicker.set(event.option.value as string);
    this.loadData();
  }
  ...
  private loadData(): void {
    ...
    const to = new Date();
    const from = new Date(to.getTime() - RANGE_DAYS[this.range()] * 86_400_000);
    ...
    this.api.getQuote(ticker).subscribe({
      ...
      error: () => this.quote.set(null), // quote is best-effort (needs live Yahoo access)
    });
  }
